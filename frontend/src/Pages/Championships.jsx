// Lista dei campionati che sono presenti nel nostro sistema
...
const ChampionshipList = () => {
    ...
    const fetchChampionships = async () => {
      try {
        const response = await fetch(
          "http://localhost:3000/api/championships/get",
        );
        if (!response.ok) {
      ...
      setChampionships((prevChampionships) =>
        prevChampionships.map((champ) =>
          champ.id === updatedChampionship.id
            ? {
                ...champ,
                members: Array.isArray(champ.members)
                  ? [...champ.members, updatedChampionship.newMember]
                  : [updatedChampionship.newMember],
              }
            : champ,
        ),
      );
  ...
  const participateInChampionship = async (championshipId) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/championships/${championshipId}/participate`,
        {
          method: "POST",
          headers: {
      ...
      socket.emit("championshipUpdated", {
        id: championshipId,
  ...
  const indexOfFirstChampionship =
    indexOfLastChampionship - championshipsPerPage;
  const currentChampionships = championships.slice(
    indexOfFirstChampionship,
    indexOfLastChampionship,
  );
          ...
          <div className="relative overflow-x-auto w-full max-w-4xl mx-auto">
                ...
                {currentChampionships.map((championship) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <button
                        onClick={() =>
                          participateInChampionship(championship.id)
                        }
            ...
            <nav className="flex justify-center mt-4">
              <ul className="inline-flex items-center -space-x-px">
                {[
                  ...Array(
                    Math.ceil(championships.length / championshipsPerPage),
                  ).keys(),
                ].map((number) => (
                  <li key={number}>
                    <button
